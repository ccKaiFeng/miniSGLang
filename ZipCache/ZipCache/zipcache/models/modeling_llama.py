...
"""PyTorch LLaMA model with ZipCache modifications.

这个文件大部分来自 HuggingFace Transformers 的 LLaMA 实现。ZipCache 主要改动：
- 在 attention 中接收 compress_config；
- past_key_value 不再只能是普通 tensor，也可以是压缩对象；
- decode 前先 decompress 历史 KV；
...
class LlamaAttention(nn.Module):
    ...
    这个类保留了普通 attention 逻辑，同时支持用 CompressUnion 压缩整个 KV。
    ZipCache 论文中的 mixed precision 重点在 MixedLlamaAttention。
    """

    def __init__(self, config: LlamaConfig, compress_config=None, layer_idx=0):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        **kwargs,
        ...
        如果 past_key_value 是压缩对象，会先 decompress 再与当前 token 的 K/V
        拼接。计算完 attention 后，如果 use_cache=True，再把 K/V 压缩保存。
        """
        ...
        if past_key_value is not None:
            ...
            if self.compress_config is not None:
                # ZipCache 路径：历史 KV 存在 CompressUnion 中，attention 计算前
                # 必须先恢复成普通 tensor。
        ...
        # if key_states.shape[2] % 20 == 0:
        #     if self.layer_idx % 10 == 0:
        #         print("min_max of key_states:",key_states.min(),key_states.max())
        #         print("min_max of value_states:",value_states.min(),value_states.max())
        ...
        if use_cache:
            if self.compress_config is not None:
                if past_key_value is None:
                    # 第一次 prefill 后创建压缩对象，把完整 prompt KV 压缩保存。
                    ...
                    # decode 阶段复用原压缩对象，重新压缩拼接后的历史 KV。
...
class MixedLlamaAttention(nn.Module):
    """ZipCache 的 LLaMA attention 主实现。

    与普通 LLaMA attention 相比，它多做三件事：
    1. prefill 阶段用 probe attention 找出低重要性 token；
    2. Key/Value 可以使用不同 mixed precision 压缩配置；
    3. streaming 模式下每隔 streaming_gap 更新一次低重要性 token 集合。
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        """执行 ZipCache attention。

        输入 hidden_states 先投影成 Q/K/V。若有历史 KV，则先解压并拼接。随后：
        - q_len > 1 表示 prefill，用部分 probe token 估计哪些历史 token 不重要；
        - q_len == 1 表示 decode，定期根据新 attention 更新 unimportant ids；
        - 真实 attention 输出仍由 FlashAttention 计算；
        - 最后把 KV 压缩成 MixedPrecisionCompressUnion 保存到 past_key_value。
        """
        ...
        if past_key_value is not None:
            # reuse k, v, self_attention
            if self.compress_config is not None:
                # 历史 KV 以压缩对象保存；每次 attention 前恢复成普通 tensor。
                prev_keys = past_key_value[0].decompress()
                prev_values = past_key_value[1].decompress()
        ...
        if q_len > 1: ## prefill phase, identify salient tokens
            # prefill 阶段 token 多，完整算所有 token 的重要性开销大。
            # 这里抽取最近 5% token + 随机 5% token 作为 probe，近似估计全 prompt
            # 中哪些历史 token 收到的注意力较少。
            # probe_ids = torch.randint(0, q_len, (int(q_len*0.1),)) ## use rand tokens
            ...
            # print(bsz, self.num_heads//self.num_key_value_groups, self.num_key_value_groups, len(probe_ids), kv_seq_len)
            attn_weights_less_heads = attn_weights.reshape(bsz, self.num_heads//self.num_key_value_groups, self.num_key_value_groups, len(probe_ids), kv_seq_len).sum(2)
            token_attn_sum = torch.sum(attn_weights_less_heads, dim=[2]) ## for fullcache
            # 越靠前的 token 可被更多后续 token 看到，直接求和会偏向早期 token。
            # 这里用一个递减因子做粗略归一化。
            token_attn_sum = token_attn_sum / torch.flip(torch.arange(1,token_attn_sum.shape[-1]+1, device=token_attn_sum.device), dims=(0,)).unsqueeze(0).unsqueeze(0)
            # 注意力最小的一部分 token 认为不重要，后续用更低 bit 压缩。
        ...
        if self.counter != 0 and self.counter % self.compress_config["streaming_gap"] == 0: ## need compress every streaming_gap tokens
            # streaming decode 中每隔 streaming_gap 个新 token，重新统计最近这段
            # token 的注意力，扩展 unimportant_ids。
        ...
        # 真实 attention 输出仍然用 FlashAttention 计算，ZipCache 不改变 attention
        # 数学公式，只改变历史 KV 的存储格式。
        attention_mask = None
        ...
        if use_cache:
            if self.compress_config is not None:
                if past_key_value is None:
                    # 第一次 cache：为 Key 和 Value 分别创建 mixed precision 压缩对象。
                    ...
                    # Key 使用 unimportant_ids_k；Value 使用 unimportant_ids_v。
                    ...
                    # 后续 decode：沿用已有压缩对象，把当前完整 KV 重新压缩进去。
                    past_key_union, past_vaule_union = past_key_value
                    past_key_union.compress(key_states, self.unimportant_ids_k)
                    if isinstance(past_vaule_union, MixedPrecisionCompressUnion):
                        past_vaule_union.compress(value_states, self.unimportant_ids_v)
...
class MyLlamaDecoderLayer(nn.Module):
    """ZipCache 版 LLaMA decoder layer。

    与原始 LlamaDecoderLayer 的主要区别是 self_attn 换成 MixedLlamaAttention，
    因此每层都能使用 compress_config 压缩自己的 KV cache。
    """
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig, compress_config=None):
        ...
        self.layers = nn.ModuleList(
            [
                LlamaDecoderLayer(config, compress_config=compress_config, layer_idx=_)
...
class MyLlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig, compress_config=None):
        ...
        self.layers = nn.ModuleList(
            [
                ...
                # 是每层自己的 past_key_value，不会跨层共享。
                MyLlamaDecoderLayer(config, compress_config=compress_config, layer_idx=_)
                for _ in range(config.num_hidden_layers)
            ]
        )
        self.norm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # past_key_value 会传入每层 attention。开启 ZipCache 时，它是一对
...
class MyLlamaForCausalLM(LlamaPreTrainedModel):
    """对外使用的 ZipCache LLaMA CausalLM 类。

    它保持 HuggingFace generate()/from_pretrained() 的使用方式，只是在内部
    MyLlamaModel 中替换了 decoder layer。
    """
    ...
    def __init__(self, config, compress_config):
        super().__init__(config)
        # compress_config 从 from_pretrained(..., compress_config=...) 传入，
        # 最终会下发到每层 MixedLlamaAttention。
        self.model = MyLlamaModel(config, compress_config=compress_config)
        self.vocab_size = config.vocab_size
